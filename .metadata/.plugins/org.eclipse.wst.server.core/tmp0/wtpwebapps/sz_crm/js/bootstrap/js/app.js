...
function callProcess(responce){
	...
	switch(responce)
	{
		...
		btn_state_change('ztth_callouter_btn',BUTTON_ON);
		btn_state_change('ztth_chanspy',BUTTON_ON);
		break;
...
//	var callouter_caller = $('#callouter_caller').val();
	var callouter_callerId=$('#ztth_callerId_num').val();//自选主叫号码yx
	var flag = $('#ztth_out_inner_flag').val();
	if(isNaN(CallNumber)){
		alert('号码错误');
		return;
	...
	if (flag == 1) {
		...
		wincall.fn_dialouter(CallNumber,callouter_callerId,_skill_group,user_channel);
	} else if(flag == 2) {
		wincall.fn_consultouter(CallNumber,callouter_callerId);
	}
	$('#ztth_myModal').modal('hide');	
}

function clickOutNumKefu(){
	CallNumber = $.trim($('#ztth_callouter_num').val());
	var callouter_caller = $('#callouter_caller').val();
//	var callouter_callerId=$('#ztth_callerId_num').val();//自选主叫号码yx
	var flag = $('#ztth_out_inner_flag').val();
