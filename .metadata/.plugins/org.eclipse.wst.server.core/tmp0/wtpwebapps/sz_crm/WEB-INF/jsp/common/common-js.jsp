...
function pageNow(pageNow){
	...
	if(pageNow < 1){
		$.messager.alert("提示信息"," 不 好 意 思 ， 已 经 是 第 一 页 啦  ！");
			return false ;
		}else if(pCount < pageNow){
			$.messager.alert("提示信息"," 没 有 下 一 页 啦 ！");
		return false ;
	}else{
...
function deleteAll(){
	...
	f=f.substring(0,f.lastIndexOf("/")+1);
		if(result() != false)
		{
			$.messager.alert("操作提示"," 请 选 择 你 要 删 除 的 项 ！");
		}else
		{
			$.messager.confirm('操作提示'," 你 确  定 要 全 部 删 除 吗 ！删 除 后 不 可 恢 复 !",function(r){
				if(r){
	...
	function deleteId(url){
		$.messager.confirm('操作提示'," 你 确  定 要 删 除 吗 ！删 除 后 不 可 恢 复 !",function(r){
			if(r){
