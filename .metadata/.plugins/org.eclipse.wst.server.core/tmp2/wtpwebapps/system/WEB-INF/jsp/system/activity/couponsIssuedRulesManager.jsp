...
<body>
	<table id="couponsIssuedRulesList" title="系统发放优惠券规则设置" class="easyui-datagrid"
		...
		<thead>
			<tr>
				...
				<th data-options="field:'type'" width="8%" formatter="formatType">赠送类型</th>
				<th data-options="field:'name'" width="10%">活动名称</th>
				...
				<th data-options="field:'couponTypeName'" width="10%" >优惠券类型</th>
				<th data-options="field:'coupons'" hidden="hidden" >优惠券ids</th>
				<th data-options="field:'couponNames'" width="25%" >优惠券名称</th>
				<th data-options="field:'status'" width="8%" formatter="formatStatus">状态</th>
				<th data-options="field:'message'" width="20%" >站内信</th>
	...
	<div id="couponsIssuedRulesTools" style="padding:5px;height:750">
		<form id="couponsIssuedRulesForm">
			赠送类型：<select id="searchType" name="type" style="width:100px;" class="easyui-combobox">
				<option value=''>全部</option>
				<option value='0'>注册赠送</option>
				<option value='1'>投资后赠送</option>
				<option value='2'>手动发送</option>
				<option value='3'>抽奖赠送1</option>
				<option value='4'>抽奖赠送2</option>
			</select> 
		...
		<form id="addIssuedRulesForm">
			<table align="center">
				...
				<tr id="range">
					<td>产品期限:</td>
					<td>
						<input id="minDeadline" name="minDeadline" style="width: 60px" class="easyui-numberbox" />至
						<input id="maxDeadline" name="maxDeadline" style="width: 60px" class="easyui-numberbox" />天
					</td>
					<td>投资金额:</td>
					<td>
						<input id="minAmount" name="minAmount" style="width: 60px" class="easyui-numberbox" data-options="min:0,max:50000000,precision:2" />至
						<input id="maxAmount" name="maxAmount" style="width: 60px" class="easyui-numberbox" data-options="min:0,max:50000000,precision:2"/>元
					</td>
				</tr>
				<tr>
					<td>开始时间≥:</td>
	...
	<div id="drManuallySendCouponTools" style="padding:5px;height:750">
	  	<form id="drManuallySendCouponForm">
	  		<div>
		    	...
		    	<a id="sendCoupon" href="javascript:updateSendCouponStatusBtn()" class="easyui-linkbutton" iconCls="icon-no">一键失效</a>
	  		</div>
	  		<div>
...
<script type="text/javascript">
        ...
        $("#addType").combobox({
            onChange: function (o,n) {
                if(o==1){
                    ...
                    $("#minDeadline").textbox('setValue',"");
                    $("#maxDeadline").textbox('setValue',"");
	...
	//一键失效所有未发送用户
	function updateSendCouponStatusBtn(){
		var validate = $("#drManuallySendCouponForm").form("validate");
		if(!validate){
			return false;
		}
		$.ajax({
          	url: "${apppath}/activity/updateSendCouponStatus.do",
            type: 'POST',
