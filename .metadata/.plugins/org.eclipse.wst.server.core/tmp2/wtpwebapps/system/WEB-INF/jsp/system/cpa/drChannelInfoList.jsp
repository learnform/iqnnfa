...
<body>
	<table id="drChannelInfoList" title="渠道列表" 
	...
	data-options="singleSelect:true,
	    ...
	    <tr>
	        ...
	        <th data-options="field:'alias'" width="10%">渠道别名</th>
	...
	<div id="addDrChannelInfoDialog" class="easyui-dialog" title="渠道添加"
		data-options="iconCls:'icon-edit',closed:true,modal:true,minimizable:false,resizable:true,maximizable:false,buttons:'#addDrChannelInfoBtn'" style="width:400px;height:190px;padding:5px;">
		<form id="addDrChannelInfoForm">
			<table align="center">
				<tr>
					...
					<td>
						<input type="text" name="code"  class="easyui-textbox"  data-options="required:true,validType:'noChinese'" />
					</td>
				</tr>
				<tr>
					<td>
				...
				<tr>
					...
					<td>		
						<input class="easyui-textbox" name="alias" data-options="required:true"/>
					</td>
				</tr>
				<tr>
					<td>是否参与推荐首投返利：</td>
	...
	<div id="updateDrChannelInfoDialog" class="easyui-dialog" title="渠道修改"
		data-options="iconCls:'icon-edit',closed:true,modal:true,minimizable:false,resizable:true,maximizable:false,buttons:'#updateDrChannelInfoBtn'" style="width:400px;height:190px;padding:5px;">
		<form id="updateDrChannelInfoForm">
			...
			<table align="center">
				...
				<tr>
					...
					<td>		
						<input id="drChannelInfoAlias" class="easyui-textbox" name="alias" data-options="required:true"/>
					</td>
				</tr>
				<tr>
					<td>是否参与推荐首投返利：</td>
	...
	function addDrChannelInfo(){  
		...
		if(!validate){
   		...
   		$.ajax({
            ...
            type: 'POST',
            data:$("#addDrChannelInfoForm").serialize(),  
		...
		$.ajax({
			...
			success:function(result){
				if(result.success){
					...
					$("#drChannelInfoAlias").textbox('setValue',result.map.drChannelInfo.alias);	
	...
	function updateDrChannelInfo(){  
		...
		if(!validate){
   		...
   		$.ajax({
            ...
            type: 'POST',
            data:$("#updateDrChannelInfoForm").serialize(),  
            success:function(result){
				if(result.success){
					$.messager.alert("提示信息",result.msg);
					$("#drChannelInfoList").datagrid("reload");
	...
	function formatStatus(value,row,index){
		if(row.status == "1"){
			...
			if(row.keyword==null||row.keyword==""||typeof(row.keyword)=="undefind"){
				...
				return '<a href="#" onclick="updateDrChannelKeyWordStatus('+row.id+',1)">失效</a>';
			}
		}
	}
