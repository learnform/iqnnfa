				...
				<tr>
    				...
    				<td>
    					<select id="addContainers" name="containers" style="width:150px;" class="easyui-combobox" data-options="required:true">
							<option value='1'>iOS_1</option>
							<option value='2'>Android</option>
							<option value='5'>iOS_2</option>
						</select>
    				</td>
				</tr>
				<tr>
...
<script type="text/javascript">
	...
	function formatOper(val,row,index){
		if(row.status==1){
			return '<a href="#" class="btn l-btn l-btn-small" onclick="showAppRenewalBtn('+index+')">查看</a>'+"  "+
				...
				'<a href="#" class="btn l-btn l-btn-small" onclick="deleteAppRenewalBtn('+index+')">失效</a>';
	...
	function deleteAppRenewalBtn(index) {
		$('#appRenewalList').datagrid('selectRow',index);// 关键在这里 
	    var row = $('#appRenewalList').datagrid('getSelected'); 
        $.messager.confirm('操作提示', '确定要使其失效吗?', function(r){
        	if(r){
	            $.ajax({
	            	url: "${apppath}/app/deleteAppRenewal.do?id="+row.id,
	                type: 'POST',
	               success:function(result){
						if(result.success){
							$.messager.alert("提示信息",result.msg);
